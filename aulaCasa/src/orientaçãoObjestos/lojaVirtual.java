...
public class lojaVirtual {
	public static void main(String [] args) {
			...
			case 5: 
				...
				if ( tenis == 1 ) {
					...
					if ( compra == 1 ) {
						...
						cont6++;
						System.out.println(" Produto adicionado ao carrinho de compras! ");	
						valor5 = (int) (cont6 * 500); 
					}
					
				} break; 	
			}
		
		System.out.println("\n Total itens no carrinho: " + cont);
		totalCarrinho = valor1 + valor2 + valor3 + valor4 + valor5; 
		System.out.println(" o valor do carrinho de compras é: R$ "+ totalCarrinho);
				...
				if ( pagamento == 1 ) {
					...
					result = (int) formasPagar ( formaPagamento, totalCarrinho); 
		...
		public static int formasPagar ( int formaPagamento, int totalCarrinho ) {
			...
			if ( formaPagamento == 1 ) {
				...
				System.out.println(" O código da sua compra é AJNGS76AS ( Digite 1 se o pagamento já foi realizado.)");
				pagamento = sc.nextInt(); 
				
				if ( pagamento == 1) {
					System.out.println(" compra feita com sucesso! obrigada pela preferência. ");
					}
			}
			
			if ( formaPagamento == 2) {
				System.out.println(" O total da compra é: R$"+ totalCarrinho);
				System.out.println(" Quer parcelar em quantas vezes? (até 5 vezes) "
						+ "\n 1x"
						+ "\n 2x"
						+ "\n 3x"
						+ "\n 4x"
				...
				if (pagamento == 2 ) {
					parcelado = totalCarrinho / 2;
					System.out.println(" Total da compra: R$"+ parcelado);
				  do {
		                System.out.println("\n Pode incerir a senha: ");
		                	senha = sc.next();
		                if(senha.length()==4) {
